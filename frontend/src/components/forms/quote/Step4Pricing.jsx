...
export default function Step4Pricing({ data, update, onNext, onBack }) {
  const pricing = useMemo(() => calculateQuotePricing(data), [
    data.materials,
    data.fabricationItems,
    data.embroideryItems,
    data.printItems,
    data.embroideryCost,
    data.printCostPerPiece,
    data.printCost,
    data.urgent,
    data.markup,
  ...
  const [erpPrice, setErpPrice] = useState(null);
  const [loadingErp, setLoadingErp] = useState(false);
  const fetchRef = useRef(null);

  const options = data.erpMarkupOptions || [];
  const hasOptions = options.length > 0;
  const selectedMarkup = data.erpMarkup;

  useEffect(() => {
    if (!data.reference || !selectedMarkup?.codigo) {
      setErpPrice(null);
      return;
    }
    ...
    fetchRef.current = setTimeout(async () => {
      setLoadingErp(true);
      try {
        const { data: formation } = await productsAPI.getFormacaoPreco(data.reference, selectedMarkup.codigo);
        setErpPrice(formation?.precoVenda ?? null);
      } catch {
        setErpPrice(null);
      } finally {
        setLoadingErp(false);
  ...
  }, [data.reference, selectedMarkup?.codigo]);

  const selectMarkupOption = (markup) => {
    update({
      erpMarkup: markup,
      markupCoeficiente: markup.coeficiente ?? null,
      markupSource: 'ERP',
    });
  };
  ...
  const costBreakdown = [
    { label: 'Materia-prima', value: pricing.totalMaterial },
    ...(pricing.fabricFreight > 0 ? [{ label: 'Frete tecidos/malhas (3%)', value: pricing.fabricFreight }] : []),
    { label: 'M.O. / Fabricacao', value: pricing.totalFabrication },
    ...(pricing.embroideryCost > 0 ? [{ label: 'Bordado', value: pricing.embroideryCost }] : []),
    ...(pricing.printCost > 0 ? [{ label: 'Estampa/Sublimacao', value: pricing.printCost }] : []),
    ...(pricing.urgencyCost > 0 ? [{ label: 'Acrescimo de urgencia', value: pricing.urgencyCost }] : []),
  ];

  return (
    <div className="space-y-5">
      <div>
        <h2 className="text-base font-semibold text-gray-900">Etapa 4 - Precificacao e Margem</h2>
      ...
      {hasOptions ? (
        <div className="p-4 bg-blue-50 border border-blue-200 rounded-xl space-y-3">
          ...
          <div className="grid gap-2">
            {options.map((markup) => {
              const isSelected = selectedMarkup?.codigo === markup.codigo;

              return (
                <button
                  key={markup.codigo}
                  type="button"
                  onClick={() => selectMarkupOption(markup)}
                    ...
                    <div>
                      ...
                      {markup.coeficiente && (
                        <p className={`text-xs mt-0.5 ${isSelected ? 'text-blue-200' : 'text-gray-500'}`}>
                          Coeficiente {markup.coeficiente.toFixed(4)} . Soma {markup.somaIndices?.toFixed(2)}% . Margem ~
                          {(100 - 100 / markup.coeficiente).toFixed(1)}%
                        </p>
                      )}
                    </div>
                    {isSelected && <span className="text-white text-lg">OK</span>}
                  </div>

                  {isSelected && markup.indices?.length > 0 && (
                    <div className="mt-2 grid grid-cols-2 gap-1">
                      {markup.indices.map((index) => (
                        <div key={index.codigo} className="flex justify-between bg-blue-500 rounded px-2 py-0.5 text-xs">
                          <span className="text-blue-100 truncate">{index.descricao}</span>
                          <span className="font-medium text-white ml-2 shrink-0">{index.indiceNacional}%</span>
                        </div>
                      ))}
                    </div>
                  )}
      ...
      ) : selectedMarkup?.indices?.length > 0 ? (
        <div className="p-4 bg-blue-50 border border-blue-200 rounded-xl space-y-3">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-semibold text-blue-800">Markup ERP: {selectedMarkup.descricao}</p>
              <p className="text-xs text-blue-600 mt-0.5">
                Coeficiente {selectedMarkup.coeficiente?.toFixed(4)} x custo = margem de
                {' '}
                {(100 - 100 / selectedMarkup.coeficiente).toFixed(1)}% sobre o preco
              </p>
            </div>
            {data.markupSource === 'MANUAL' && (
              <button
                ...
                onClick={() => update({ markupCoeficiente: selectedMarkup.coeficiente, markupSource: 'ERP' })}
              >
                Restaurar ERP
              </button>
            )}
          ...
          <div className="grid grid-cols-2 gap-1.5 text-xs">
            {selectedMarkup.indices.map((index) => (
              <div key={index.codigo} className="flex justify-between bg-white rounded px-2 py-1 border border-blue-100">
                <span className="text-gray-600 truncate">{index.descricao}</span>
                <span className="font-medium text-blue-700 ml-2 shrink-0">{index.indiceNacional}%</span>
              </div>
            ))}
          </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        {((!hasOptions && !selectedMarkup?.indices?.length) || data.markupSource === 'MANUAL') && (
          <div>
            ...
            <input
              ...
              onChange={(event) => update({
                markup: parseFloat(event.target.value) || 0,
                markupSource: 'MANUAL',
      ...
      <div>
        <label className="label">Observacoes do Orcamento</label>
        <textarea
          className="input"
          rows={3}
          value={data.notes || ''}
          onChange={(event) => update({ notes: event.target.value })}
          placeholder="Condicoes especiais, prazos, informacoes adicionais..."
        />
      </div>

      <div className="p-5 bg-gradient-to-br from-orange-50 to-amber-50 border border-orange-200 rounded-xl">
        <p className="text-xs font-semibold text-orange-700 uppercase tracking-wide mb-3">Resultado da Precificacao</p>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="bg-white rounded-lg p-3 border border-orange-100">
            <p className="text-xs text-gray-500">Preco Sugerido (local)</p>
            <p className="text-xl font-bold text-orange-700">R$ {pricing.pricePerPiece.toFixed(2)}</p>
          </div>
          ...
          {(erpPrice !== null || loadingErp) && (
            <div className="bg-blue-50 rounded-lg p-3 border border-blue-100">
              <p className="text-xs text-blue-600">Preco ERP ({selectedMarkup?.descricao})</p>
        ...
        {pricing.margin < 25 && pricing.margin > 0 && (
          <div className="mt-3 p-2 bg-red-50 border border-red-200 rounded-lg">
            <p className="text-xs text-red-700 font-medium">
              Margem abaixo de 25% - considere revisar o markup antes de enviar para aprovacao
            </p>
          </div>
        )}
      </div>
